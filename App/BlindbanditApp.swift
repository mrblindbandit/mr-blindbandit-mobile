    ...
    func unlock() {
        ...
        ctx.evaluatePolicy(.deviceOwnerAuthentication, localizedReason: "Unlock your private Blindbandit app") { success, _ in
            Task { @MainActor in
                ...
                } else { self.message = "Unlock was cancelled or unsuccessful. You can try again." }
            }
        }
    }
}
...
@main struct BlindbanditApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            ZStack {
                if lock.unlocked {
                    Tabs().environmentObject(lock).accessibilityHidden(phase != .active)
                } else {
                    VStack(spacing: 24) {
                        ...
                        Text(lock.configured ? "Blindbandit is locked" : "Protect your private app")
                            .font(.title).accessibilityAddTraits(.isHeader)
                        ...
                        Button(lock.configured ? "Unlock app" : "Set up secure unlock") { lock.unlock() }
                            .buttonStyle(.borderedProminent).disabled(lock.busy)
                        if lock.busy { ProgressView("Waiting for device authentication") }
                        Text(lock.message).foregroundStyle(.secondary)
                    }.padding(28)
                }
                if phase != .active {
            ...
            }.onChange(of: phase) { _, value in if value == .background { lock.lock() } }
        }
    }
}
...
struct Home: View {
    var body: some View {
        List {
            Section {
                Text("Mr. Blindbandit").font(.largeTitle).bold().accessibilityAddTraits(.isHeader)
                Text("Blindbandit Records · Private companion")
            }
    ...
    var body: some View {
        Form {
            ...
            Section("Accessibility") {
                Text("Native controls support VoiceOver and your preferred text size. Appearance follows your device settings. Website accessibility depends on each page.")
                Button("Open app settings") { UIApplication.shared.open(URL(string: UIApplication.openSettingsURLString)!) }
            }
            Section("Account") {
                NavigationLink("Account and security settings") { Website(path: "/account", title: "Account") }
                Text("Your Clerk session is retained by iOS WebKit until it expires, is revoked, or is cleared. No server secrets are bundled.")
            }
            Section("About") {
                Text("Blindbandit private companion 1.0")
                Text("Native navigation with connected website screens. Internet connection required.")
            }
...
@MainActor final class Browser: NSObject, ObservableObject, WKNavigationDelegate, WKUIDelegate {
    ...
    init(path: String) {
        initialURL = URL(string: "https://mrblindbandit.net" + path)!
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        loading = false; back = webView.canGoBack; forward = webView.canGoForward
    }
    ...
    func webViewWebContentProcessDidTerminate(_ webView: WKWebView) {
        loading = false; error = "The page stopped responding. Reload to continue. Unsaved changes may be lost."
    }
    func firstParty(_ url: URL) -> Bool {
        url.scheme == "https" && ["mrblindbandit.net", "www.mrblindbandit.net", "portal.mrblindbandit.net", "clerk.mrblindbandit.net", "accounts.mrblindbandit.net"].contains(url.host?.lowercased() ?? "")
    }
    func webView(_ webView: WKWebView, decidePolicyFor action: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        guard let url = action.request.url else { decisionHandler(.cancel); return }
        if action.targetFrame?.isMainFrame == false || firstParty(url) { decisionHandler(.allow); return }
    ...
    func webView(_ webView: WKWebView, createWebViewWith configuration: WKWebViewConfiguration, for action: WKNavigationAction, windowFeatures: WKWindowFeatures) -> WKWebView? {
        ...
        if firstParty(url) { webView.load(action.request) }
        else if url.scheme == "https" { UIApplication.shared.open(url) }
        return nil
    }
