...
final class AudioConverterModel: ObservableObject {
    ...
    func setInput(_ url: URL) {
        ...
        outputURL = nil
        progressText = inputURL == nil ? "The selected file could not be opened." : "Ready to convert."
    }
    ...
    private func convertPCM(_ input: URL, bitDepth: Int) throws -> URL {
        ...
        if bitDepth == 24 { settings[AVLinearPCMIsAlignedHighKey] = false }
...
struct NativeAudioConverterView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ToolHero(icon: "waveform.badge.plus", title: "Audio Converter", subtitle: "Professional local conversion. Your audio stays on this iPhone.")

                VStack(alignment: .leading, spacing: 14) {
                    Label("Source", systemImage: "music.note")
                        .font(.headline)
                    Button(model.inputURL?.lastPathComponent ?? "Choose audio file") { showPicker = true }
                    ...
                    Button("Convert audio") { Task { await model.convert() } }
                        .buttonStyle(.borderedProminent)
        ...
        .fileImporter(isPresented: $showPicker, allowedContentTypes: [.audio]) { result in
            if case let .success(url) = result { model.setInput(url) }
            if case let .failure(error) = result { model.progressText = error.localizedDescription }
        }
    }
}
...
final class ArtTrackGeneratorModel: ObservableObject {
    ...
    func setArtwork(_ url: URL) { artworkURL = copy(url); outputURL = nil }
    func setAudio(_ url: URL) { audioURL = copy(url); outputURL = nil }
    ...
    func generate() async {
        ...
        do {
            ...
            let silentVideo = try await renderStillVideo(image: image, duration: duration, size: canvas.size, waveform: showWaveform)
    ...
    private func renderStillVideo(image: UIImage, duration: CMTime, size: CGSize, waveform: Bool) async throws -> URL {
        let output = FileManager.default.temporaryDirectory.appendingPathComponent("arttrack-video-\(UUID().uuidString).mp4")
        ...
        while !input.isReadyForMoreMediaData { try await Task.sleep(for: .milliseconds(20)) }
        adaptor.append(pixelBuffer, withPresentationTime: .zero)
        let end = CMTimeSubtract(duration, CMTime(value: 1, timescale: 30))
        if end > .zero {
            while !input.isReadyForMoreMediaData { try await Task.sleep(for: .milliseconds(20)) }
            adaptor.append(pixelBuffer, withPresentationTime: end)
        }
    ...
    private func makePixelBuffer(image: UIImage, size: CGSize, waveform: Bool, pool: CVPixelBufferPool?) throws -> CVPixelBuffer {
        ...
        if waveform {
            ...
            let widths: [CGFloat] = [0.18, 0.34, 0.54, 0.28, 0.72, 0.46, 0.22, 0.62, 0.38, 0.16]
            context.setFillColor(UIColor.white.withAlphaComponent(0.92).cgColor)
            let total = size.width * 0.6
            let barWidth = total / CGFloat(widths.count * 2)
    ...
    private func aspectFit(_ image: CGSize, inside canvas: CGSize) -> CGRect {
        ...
        return CGRect(x: (canvas.width - fitted.width) / 2, y: (canvas.height - fitted.height) / 2, width: fitted.width, height: fitted.height)
    }
    ...
    private func merge(video: URL, audio: AVURLAsset, duration: CMTime) async throws -> URL {
        ...
        let output = FileManager.default.temporaryDirectory.appendingPathComponent("Mr-Blind-Bandit-Art-Track-\(UUID().uuidString).mp4")
    ...
    private func copy(_ url: URL) -> URL? {
        let accessed = url.startAccessingSecurityScopedResource()
        defer { if accessed { url.stopAccessingSecurityScopedResource() } }
        let destination = FileManager.default.temporaryDirectory.appendingPathComponent("media-\(UUID().uuidString)-\(url.lastPathComponent)")
        do {
            try? FileManager.default.removeItem(at: destination)
            try FileManager.default.copyItem(at: url, to: destination)
            return destination
...
struct NativeArtTrackGeneratorView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ToolHero(icon: "rectangle.stack.badge.play", title: "Art Track Generator", subtitle: "Create a finished MP4 locally from your artwork and audio.")

                VStack(alignment: .leading, spacing: 14) {
                    Button(model.artworkURL?.lastPathComponent ?? "Choose artwork") { pickArtwork = true }
                        .buttonStyle(.borderedProminent)
                    Button(model.audioURL?.lastPathComponent ?? "Choose audio") { pickAudio = true }
                        .buttonStyle(.borderedProminent)
                    ...
                    Button("Generate art track") { Task { await model.generate() } }
                        .buttonStyle(.borderedProminent)
                ...
                ToolInfoCard(title: "Local rendering", lines: [
                    ...
                    "Highest-quality MP4 export available through AVFoundation",
        ...
        .fileImporter(isPresented: $pickArtwork, allowedContentTypes: [.image]) { result in
            if case let .success(url) = result { model.setArtwork(url) }
        ...
        .fileImporter(isPresented: $pickAudio, allowedContentTypes: [.audio]) { result in
            if case let .success(url) = result { model.setAudio(url) }
            if case let .failure(error) = result { model.status = error.localizedDescription }
        }
    }
}
...
struct ToolHero: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            Text(title).font(.largeTitle.bold()).frame(maxWidth: .infinity, alignment: .leading)
            Text(subtitle).font(.body).foregroundStyle(.secondary).frame(maxWidth: .infinity, alignment: .leading)
        }
    }
}
